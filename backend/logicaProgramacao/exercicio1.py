#eu, como dono de uma padaria quero um sistema aonde eu possa cadastrar meus 
#produtos alem de poder listar,alterar,em caso de erro e excluir quando acabar
#o estoque quero tambem que tenha um menu onde eu possa ver as opções possiveis em python
...
def listar():                                           #FAZ ESSE "DEF" PRA APARECER O MENU CADASTRAR QUANDO EXECUTAR DEPOIS ELE VAI TE PEDIR O NOME COM O INPUT
    if not produtos:
        ...
        for nome in produtos:             #Esse é um laço for com enumerate(), usado para percorrer a lista de produtos.
            print(f"{nome}")                      #Mostra o índice (ID) e o nome do produto
...
def excluir():
    ...
    try:
        ...
        if 0 <= i < len(produtos):                       #Verifica se o número digitado (i) está dentro de uma faixa válida de índices da lista produtos. e len Garante que i não ultrapassa o tamanho da lista.
